...
impl StreamMux {
    pub fn new<T: AsyncRead + Send + Unpin + 'static, U: AsyncWrite + Send + Unpin + 'static>(
        ...
        // Deadlock fix (vox-car 2026-09-20): `chan` carries both frames-to-decrypt and
        // frames-to-encrypt; the consumer writes acks into it while the TLS task waits to
        // hand it decrypted frames. With both bounded, duplex load closes the cycle. The
        // TLS->consumer leg is unbounded so the TLS task can always drain `chan`.
